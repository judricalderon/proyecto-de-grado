...
def lambda_handler(e,c):
 try:
  ...
  if path.endswith("/fases") and path.startswith("/modulos/"):
   items=service.list_items("fases",q.get("id_modulo"));return out(200,{"data":items,"count":len(items)})
  if not kind:return out(404,{"error":"Ruta no encontrada","code":"ROUTE_NOT_FOUND","details":[]})
  item_id=q.get("id")
  if path==f"/{kind}":
   if m=="GET":
    items=service.list_items(kind);return out(200,{"data":items,"count":len(items)})
